...
const BlogDetay = () => {
    ...
    return (
      <div className="min-h-screen flex flex-col">
        ...
        <div className="flex-1 flex items-center justify-center flex-col gap-4 pt-24">
          ...
          <Link to="/blog"><Button variant="outline"><ArrowLeft className="h-4 w-4 mr-2" /> Blog'a Dön</Button></Link>
        </div>
        <Footer />
      </div>
    );
  ...
  return (
    <div className="min-h-screen flex flex-col">
      <Helmet>
        <title>{post.title} | Peyzaj Rehberi Blog</title>
        <meta name="description" content={post.excerpt || `${post.title} - Peyzaj Rehberi Blog`} />
        <script type="application/ld+json">{JSON.stringify({
          ...
          "publisher": { "@type": "Organization", "name": "Peyzaj Rehberi" },
        })}</script>
      </Helmet>
      <Navbar />
